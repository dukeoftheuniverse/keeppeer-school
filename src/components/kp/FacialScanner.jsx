 ...
 * The live camera is always visible. Operator searches + selects a person,
 * then taps Verify — the camera confirms that person's face and records
 * attendance via onMatch(person, confidence).
 *
 * Props: enrolledPeople, onMatch(person, confidence), disabled
 */
...
export default function FacialScanner({ enrolledPeople = [], onMatch, disabled }) {
  ...
  const frameColor = phase === 'scanning' ? 'border-yellow-400 animate-pulse'
    : phase === 'success' ? 'border-green-400'
    : phase === 'fail' ? 'border-red-400' : 'border-white/70';

  return (
    <div>
      <p className="text-sm text-gray-500 mb-3 flex items-center gap-1.5"><ScanFace className="w-4 h-4 text-[hsl(var(--kp-teal))]" /> Turn on the camera, select the person, then verify their face.</p>
      ...
      {phase !== 'success' && phase !== 'fail' && (
        <KpButton variant="teal" onClick={verify} disabled={disabled || phase === 'scanning' || !selected} className="w-full justify-center">
      ...
      {(phase === 'success' || phase === 'fail') && (
        <KpButton variant="outline" onClick={reset} className="w-full justify-center">
