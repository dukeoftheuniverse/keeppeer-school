            ...
            {myClasses.length === 0 ?
              ...
              <div className="flex gap-3 overflow-x-auto kp-scroll-thin pb-2">
                {myClasses.map((item, idx) => {
                  ...
                  return (
                    <button key={c.id} onClick={() => selectClass(item)} className={`bg-white rounded-2xl shadow p-4 min-w-[190px] text-left border-2 transition-all text-xs ${active ? 'border-[#004D5A] ring-2 ring-[#004D5A]/20' : `${bc} hover:opacity-90`}`}>
