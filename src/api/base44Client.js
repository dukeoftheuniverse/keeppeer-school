/**
 * Standalone client — replaces @base44/sdk entirely.
 * Talks to the local MySQL backend (server/index.js). No Base44 SDK.
 * Configure with VITE_API_URL (default http://localhost:4000/api).
 */
...
const auth = {
  ...
  async loginViaEmailPassword(email, password) { return this.login(email, password); },
  ...
  // Stubs for parity with the old SDK surface (no email/social provider in standalone)
  async loginWithProvider() { throw new Error('Social login is not available in standalone mode.'); },
  async verifyOtp() { return { access_token: getToken() }; },
...
const integrations = {
  Core: {
    ...
    async ExtractDataFromUploadedFile() { return { status: 'error', output: null }; },
...
export const base44 = { entities, auth, users, integrations, analytics };
